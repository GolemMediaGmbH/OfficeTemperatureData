...
rs <- dbSendQuery(db, "SELECT AVG(r_temp) AS rtemp, AVG(dwd_temp) AS dwdtemp, HOUR(dt) AS hour 
                      FROM ot 
                      WHERE f_ignore = 0 AND dwd_temp IS NOT NULL AND MONTH(dt) = 1
                      GROUP BY HOUR(dt) 
...
png("generated/414_avg_temp_dwd_by_hour_in_january.png", width=1000, height=600)
ggplot(data=mdata, aes(x=hour,y=value,colour=variable, linetype=variable)) + 
  geom_line() +
  ggtitle("Durchschnittstemperaturen deutscher Messstationen im Vergleich mit DWD-Aussentemperaturen pro Stunde im Januar 2017")+  
